...
Simulating temporal *J dissimilarity* & *log Ratio* under different
scenarios of scale, occupancy, temporal change magnitude

[**Parameters to simulate:**]{.underline}
...
```{r}
#| label: create-change-for-directional-expanding
...
expanding <-
  ...
  mutate(
    trend = as.factor(case_when(b > c ~ "expanding range",
      b < c ~ "declining range",
...
```{r}
#| label: create-change_type2-column
#| message: false
#| error: false
#| warning: false
...
sim_df <-
  ...
  mutate(
    change_type2 =
      as.factor(
      case_when(change_type == "random" ~ "random",
...
```{r}
model_df <- sim_df %>% 
  select(j_dissim, log_ratio,
         universe_size, AOO_1, n_changes, change_type) %>% 
  mutate(across(c(universe_size, AOO_1, n_changes),
                scale, .names = "z_{.col}"))          # standardise for comparability

m_j <- lm(j_dissim ~ z_universe_size +
                        z_AOO_1 * z_n_changes +
                        change_type,
          data = model_df)

m_lr <- lm(log_ratio ~ z_universe_size +
                          z_AOO_1 * z_n_changes +
                          change_type,
           data = model_df)
...
summary(m_j)
m_j$coefficients %>% round(3)
summary(m_lr)
m_lr$coefficients %>% round


library(broom)

tidy(m_j, conf.int = TRUE) %>% 
  arrange(desc(abs(statistic)))   # biggest |t| first

tidy(m_lr, conf.int = TRUE) %>% 
...
### random x large subset
this one gives the clearest pattern in terms of resolution, 
and the most representable in nature because change is very rarely only directional.
...
#### log R \~ J (smaller subset: 20 runs instead of 100)
...
#### 3D surface plot for log ratio (GAM smoothed)
...
Change intensity is: the ratio between the number of changed cells and
AOO_1: change intensity = (b+c)/AOO_1 where b = colonized cells, c =
extirpated cells

Explanation: I. ratio approaches 0 when low change - i.e., when the
number of lost + gained cells is low compared to AOO_1. - b+c \< AOO_1
--\> leads to low J dissimilarity

II. ratio approaches 1 when high change

-   i.e., when the number of lost + gained cells is comparable to AOO_1
-   b+c = AOO_1 --\> leads to high J dissimilarity for both directions
    of change

III. ratio approaches 2 when change is higher than initial occupancy
...
or - i.e., when all no cells persist, i.e., all cells from AOO_1 are
lost and some are gained (c = AOO_1 & b \> 0) - a = 0 --\> leads to high
J for both directions of change or - i.e., with strong expansion (b \>\>
AOO_1) --\> leads to moderate to strong J for both directions
...
ggplot(plot_data, aes(x = .smooth, y = .estimate)) +
  ...
  labs(
    x = "a (shared occupancy)", y = "Partial effect on Jaccard dissimilarity",
...
ggplot(sim_df, aes(x = gam_mod$fitted.values, y = j_dissim)) +
  ...
  labs(
    x = "Predicted Jaccard dissimilarity",

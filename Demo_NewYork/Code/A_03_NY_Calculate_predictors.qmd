...
IUCN.list <-
  suppressMessages(iucn_summary(unique(sci_name$scientificName),
    distr.detail = F,
    key = Sys.getenv("IUCN_REDLIST_KEY") # change to your token key
  ))
...
```{r}
# Functions:
## Normalized circularity function (written by ANONYMIZED-3)
circNorm <- function(x){
    perimeter <- perim(x)
    area <- expanse(x)
...
### Spatial autocorrelation
We calculate the difference in expected and observed join counts for presences across the atlas for each species and sampling period. This is done using the `joincount.test()` function from the `spdep` package. The join count test is a measure of spatial autocorrelation that counts the number of pairs of neighboring sites that have the same value (in this case, presence or absence of a species). In the calculation of delat join count, we follow work by Soria et al. (2025) [5]
...
[5] Soria, C. D., Ortega-Solís, G. R., Wölke, F. J. R., Barták, V., Tschernosterová, K., Bejček, V., Herrando, S., Mikuláš, I., Šťastný, K., Ueta, M., Voříšek, P., & Keil, P. (Preprint, 2025). Spatial autocorrelation of species diversity and distributions in time and across spatial scales. https://doi.org/10.1101/2025.06.13.659470

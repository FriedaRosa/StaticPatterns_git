...
library(grid)
J_LR_plot <- ggplot(dat_cat %>% filter(samplingPeriodID == 1),
  ...
  annotation_custom(
    grob = rectGrob(gp = gpar(fill = "#fdae61", col = "#fdae61", alpha = 0.3)), xmin = log(0.5), xmax = log(0.9), ymin = 1, ymax = 0.1) +  # top-left
  annotation_custom(
    grob = rectGrob(gp = gpar(fill = "#a6d96a", col = "#a6d96a", alpha = 0.3)),xmin = log(1.1), xmax = log(2), ymin = 1, ymax = 0.1) +  # top-right
  annotation_custom(
    grob = rectGrob(gp = gpar(fill = "#d7191c", col = "#d7191c", alpha = 0.3)), xmin = -2, xmax = log(0.5), ymin = 1, ymax = 0.5) +  # middle-left
  annotation_custom(
...
legend <- ggplot(dat_cat %>% filter(samplingPeriodID == 1),
                 aes(y = Jaccard_dissim, x = log_R2_1, fill = trend_LR)) +
